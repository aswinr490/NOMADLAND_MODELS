...
def agent_manage(request):
    return render(request, 'agent_bookings.html')
...
def add_hotels(request):
    ...
    if request.method == 'POST':
        ...
        print(package_id,'this is package_id')
...
def update_package_details(request, package_id):
    package_obj = get_object_or_404(PackageModel, pk=package_id)

    if request.method == 'POST':
        # Retrieve data from the combined form
        nation_name = request.POST.get('nation_name')
        destination = request.POST.get('destination')
        price = request.POST.get('price')
        description = request.POST.get('activity_description')
        ...
        end_date = request.POST.get('end_date')
        quantity = request.POST.get('quantity')
        package_images = request.FILES.getlist('image')
        activity_names = request.POST.getlist('activity_name[]')
        activity_images = request.FILES.getlist('activity_image[]')
        activity_descriptions = request.POST.getlist('activity_description[]')
        hotel_names = request.POST.getlist('hotel_name[]')
        ...
        existing_nation = NationsModel.objects.filter(nation=nation_name).first()
        if existing_nation:
            nation_id = existing_nation.pk
        else:
            new_nation = NationsModel.objects.create(nation=nation_name)
            nation_id = new_nation.pk

        # Update PackageModel instance
        package_obj.nation_id = nation_id
        package_obj.destination_name = destination
        ...
        # Update PackageSplit instance
        package_split, created = PackageSplit.objects.get_or_create(package_id=package_obj)
        package_split.start_date = start_date
        package_split.end_date = end_date
        package_split.quantity = quantity
        package_split.save()

        # Save or update activities
        for name, image, desc in zip(activity_names, activity_images, activity_descriptions):
            activity, created = ActivitiesModel.objects.get_or_create(
                activities=name,
                activity_description=desc,
                activity_images=image,
                package_id_id=package_obj.pk
            )

        # Save or update hotels
        for name, price, quantity, image in zip(hotel_names, hotel_prices, hotel_quantities, hotel_images):
            hotel, created = HotelModel.objects.get_or_create(hotel_name=name)
            hotel_image, created = HotelImage.objects.get_or_create(hotel_id=hotel, hotel_image=image)
            package_hotel, created = PackageHotel.objects.get_or_create(
                hotel_id=hotel,
                package_split_id_id=package_split.pk,
                price=price,
                quantity=quantity
            )

        return redirect('display_hotels')  # Redirect to a relevant page after saving

    return render(request, 'edit_package.html', {'package': package_obj})
...
def booking_approval(request, booking_id):
    if request.method == 'POST':
        ...
        return render(request, 'agent_booking.html' )

def agent_booking(request):
    bookings = BookingModel.objects.all()
    context = {'bookings': bookings}
    return render(request, 'agent_booking.html', context)