...
def login(request):
    if request.method == 'POST':
        if 'user_signin' in request.POST:
            ...
            user_data = UserModel.objects.filter(user_email=user_email, user_password=user_password).first()
...
def review(request):
    ...
    if request.method == 'POST':
        ...
        return redirect('')  # Change 'review_success' to your success URL
...
#
#     package_hotels = PackageHotel.objects.select_related('hotel_id', 'package_split_id').all()
#
#     package_splits = PackageSplit.objects.all()
#
#     context = {
#         'hotels': hotels,
#         'hotel_images': hotel_images,
...
#     }
#
#     return render(request, 'hotel_select.html', context)
#
#


# def hotel_preview(request):
#     if request.method == 'POST':
#         package_id = request.POST.get('package_id')
...
#
#         try:
#             package_hotel_instance = PackageHotel.objects.get(package_id=package_id, hotel_id=hotel_id)
#         except PackageHotel.DoesNotExist:
#
...
#
#
#         booking.package_hotel_id = package_hotel_instance
#         booking.save()
#
#
#         return redirect('package_preview')
#
#     return redirect('display_hotels')
...
def history_page(request):
    if 'user_id' in request.session:
        user_id = request.session.get('user_id')
        user = UserModel.objects.get(user_id=user_id)  # Fetch the user
        ...
        return render(request, 'history.html', {'user': user, 'bookings': bookings})
    else:
        return redirect('login')
